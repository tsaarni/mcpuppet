      ...
      if (sessionId) {
        ...
        if (!existing) {
          ...
          if (sessions.size >= MAX_CONNECTIONS) {
            return c.json({ jsonrpc: "2.0", error: { code: -32600, message: "Server at capacity" }, id: null }, 503);
          }
          // Recovery path
...
void test("session: multiple concurrent SSE streams are allowed", async () => {
  ...
  try {
    ...
    const sse1 = fetch(url, {
      method: "GET",
      headers: { Accept: "text/event-stream", "mcp-session-id": sessionId },
      signal: ctrl1.signal,
    });
    const sse2 = fetch(url, {
      method: "GET",
      headers: { Accept: "text/event-stream", "mcp-session-id": sessionId },
      signal: ctrl2.signal,
    });
  ...
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json, text/event-stream",
        "mcp-session-id": "fake-overflow-id",
      },
