...
import { extractIPv4Compatible, validateUrlPolicy } from "../src/stages/url-policy.ts";
...
void test("pipeline composes stages", async () => {
  ...
  const pipeline: Stage[] = [new StageOne(), new StageTwo()];
...
void test("pipeline propagates stage error", async () => {
  ...
  const pipeline: Stage[] = [new ExplodeStage()];
...
void test("sanitize-and-clean combines both passes and provides document", () => {
  ...
  const input =
    '<html><body><!--secret--><p aria-hidden="true">hidden</p><header>hdr</header><nav>n</nav><main>ok</main><script>bad()</script></body></html>';
