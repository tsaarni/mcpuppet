import { detectChromeExecutable } from "./platform.ts";
...
export const config = {
  ...
  requestTimeoutMs: envNumber("MCPUPPET_REQUEST_TIMEOUT_MS", 15000),
  settleDelayMs: envNumber("MCPUPPET_SETTLE_DELAY_MS", 200),
  maxRedirects: envNumber("MCPUPPET_MAX_REDIRECTS", 5),
  searchBackend: process.env.MCPUPPET_SEARCH_BACKEND ?? "duckduckgo",
  ...
  executablePath: process.env.MCPUPPET_EXECUTABLE_PATH ?? detectChromeExecutable(), // If unset, auto-detect system Chrome; fall back to Puppeteer's bundled browser.
