...
export interface ConnectionState {
  ...
  invalidated: boolean;
  /** Timestamp of the last search to enforce rate limiting. */
  lastSearchAt: number;
}
  ...
  async getOrCreate(connectionId: string): Promise<ConnectionState> {
    ...
    if (!state) {
      if (this.connections.size >= config.maxConnections) {
        logger.warn(
          { connectionId, currentConnections: this.connections.size, maxConnections: config.maxConnections },
      ...
      state = {
        ...
        pagePromise: null,
        createdAt: new Date(),
        mutex: withTimeout(new Mutex(), config.requestTimeoutMs + config.settleDelayMs),
        invalidated: false,
        lastSearchAt: 0,
      };
