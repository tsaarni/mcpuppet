...
async function initSession(): Promise<string> {
  ...
  if (!sessionId) {
    process.stderr.write(`Error: failed to get session from ${BASE_URL}\n`);
    process.exit(1);
  }
...
async function callTool(session: string, tool: string, args: object): Promise<string> {
  ...
  return (result.result?.content ?? [])
    ...
    .map((c) => {
      try {
        const d = JSON.parse(c.text) as { contentMarkdown?: string; markdown?: string; text?: string };
        return d.contentMarkdown ?? d.markdown ?? c.text;
      } catch {
        return c.text;
      }
    })
