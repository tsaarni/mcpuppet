...
export class BrowserManager {
  ...
  async launch(): Promise<void> {
    ...
    this.launchPromise = this.doLaunch().finally(() => {
  ...
  private async doLaunch(): Promise<void> {
    ...
    for (const file of (fs.existsSync(config.userDataDir) ? fs.readdirSync(config.userDataDir) : []).filter((f) =>
      f.startsWith("Singleton"),
    )) {
      try {
        fs.unlinkSync(path.join(config.userDataDir, file));
      } catch {
        /* ignore */
      }
    }
